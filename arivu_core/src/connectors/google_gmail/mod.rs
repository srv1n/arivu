...
impl Connector for GmailConnector {
    ...
    async fn list_tools(
        ...
        let tools = vec![
            Tool { name: Cow::Borrowed("list_messages"), title: None, description: Some(Cow::Borrowed("List messages in the user's mailbox (id, threadId, snippet).")), input_schema: Arc::new(json!({"type":"object","properties":{"q":{"type":"string"},"max_results":{"type":"integer","minimum":1,"maximum":100},"response_format":{"type":"string","enum":["concise","detailed"]}},"required":[]}).as_object().expect("Schema object").clone()), output_schema: None, annotations: None, icons: None },
            Tool { name: Cow::Borrowed("decode_message_raw"), title: None, description: Some(Cow::Borrowed("Decode a Gmail raw message (base64url) and extract headers/bodies.")), input_schema: Arc::new(json!({"type":"object","properties":{"raw_base64url":{"type":"string"}},"required":["raw_base64url"]}).as_object().expect("Schema object").clone()), output_schema: None, annotations: None, icons: None },
            Tool { name: Cow::Borrowed("get_message"), title: None, description: Some(Cow::Borrowed("Get a message by id (format raw|full|metadata).")), input_schema: Arc::new(json!({"type":"object","properties":{"id":{"type":"string"},"format":{"type":"string"},"response_format":{"type":"string","enum":["concise","detailed"]}},"required":["id"]}).as_object().expect("Schema object").clone()), output_schema: None, annotations: None, icons: None },
            Tool { name: Cow::Borrowed("get_thread"), title: None, description: Some(Cow::Borrowed("Get a thread by id.")), input_schema: Arc::new(json!({"type":"object","properties":{"id":{"type":"string"}},"required":["id"]}).as_object().expect("Schema object").clone()), output_schema: None, annotations: None, icons: None },
        ];
        Ok(ListToolsResult {
            tools,
            next_cursor: None,
